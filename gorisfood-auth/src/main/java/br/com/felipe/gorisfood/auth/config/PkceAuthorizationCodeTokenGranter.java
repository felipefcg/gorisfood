...
public class PkceAuthorizationCodeTokenGranter extends AuthorizationCodeTokenGranter {
	...
	private boolean validateCodeVerifier(String codeVerifier, String codeChallenge, String codeChallengeMethod) {
		String gengeneratedCodeChallenge = null;
		...
		if ("s256".equalsIgnoreCase(codeChallengeMethod)) {
			return codeChallenge.equals(generateHashSha256(codeVerifier)); 
		}
		
		throw new InvalidGrantException(codeChallengeMethod + " is not a valid challenge method.");
	}

	private static String generateHashSha256(String codeVerifier) {
		try {
			
			var messageDigest = MessageDigest.getInstance("SHA-256");
			byte[] hash = messageDigest.digest(Utf8.encode(codeVerifier));
